...
from training.pretrain_bc import make_bc_train_step
...
def load_dataset(path: str):
    ...
    mem = MemoryState(**{f: jnp.asarray(npz[f"mem_{f}"]) for f in MemoryState._fields})
    action_index = jnp.asarray(npz["action_index"])
    return obs, mem, action_index
...
def main():
    ...
    obs, mem, action_index = load_dataset(args.data)
    num_samples = action_index.shape[0]
    print(f"  {num_samples} (observation, memory, human-action) pairs")
    ...
    for epoch in range(args.epochs):
        ...
        mem_shuf = _tree_index(train_mem, p)
        ...
        for i in range(num_batches):
            ...
            batch_mem = _tree_index(mem_shuf, sl)
        ...
        val_acc = float(_eval_accuracy(net, val_obs, val_mem, val_actions)) if num_val > 0 else float("nan")
